...
class QueryReactome:
    ...
    def query_uniprot_id_to_interacting_uniprot_ids_desc(self, uniprot_id):
        res = self.biothings_explorer.send_query_get(input_prefix='uniprot', output_prefix="uniprot", input_value=uniprot_id)
        res_uniprot_ids = dict()
        if res:
            res = [_doc for _doc in res['data'] if _doc['endpoint'] == "https://reactome.org/ContentService/interactors/static/molecule/{uniprot}/details"]
            if res:
                for res_entity_interactor in res:
                    ...
                    if 'CHEBI:' not in int_uniprot_id:
                        ...
                        if 'secondary-id' in res_entity_interactor['output']['object']:
                            int_alias = res_entity_interactor['output']['object']['secondary-id'][len('hgnc.symbol')+1:]
                        else:
                            int_alias = ''
                        alt_species = None
                        if ' ' in int_alias:
                            ...
                            if alt_species is not None:
                                if 'DNA' in int_alias_split or \
                                   ...
                                   any(['BIND' in alias_element for alias_element in int_alias_split]):                                               
                                    ...
                                    int_alias = None
                        if int_alias is not None and int_alias != "" and self.is_valid_uniprot_accession(int_uniprot_id):
                                res_uniprot_ids[int_uniprot_id] = int_alias
        return res_uniprot_ids

    def __query_uniprot_to_reactome_entity_id(self, uniprot_id):
        res = self.biothings_explorer.send_query_get(input_prefix='uniprot', output_prefix="reactome.complex", input_value=uniprot_id)
        if res:
    ...
    def __query_reactome_entity_id_to_reactome_pathway_ids_desc(self, reactome_entity_id):
        res = self.biothings_explorer.send_query_get(input_prefix='reactome.complex', output_prefix="reactome.pathway", input_value=reactome_entity_id)
        reactome_ids_dict = dict()
        if res:
            for _doc in res['data']:
                res_id = _doc['output']['object']['id']
    ...
    def query_reactome_pathway_id_to_uniprot_ids_desc(self, reactome_pathway_id):
        res = self.biothings_explorer.send_query_get(input_prefix='reactome.pathway', output_prefix="uniprot", input_value=reactome_pathway_id)
        ret_dict = dict()
        if res:
